...
class Job58Corp(Corp):
    def __init__(self):
        config = {
            'info_from': '58同城',
            'corplist_url': 'http://{0}.58.com/job/pn{1}/?param9047=1',
            'corp_url': '{corp_code}',
            'corplist_post_data': None,
            'corp_post_data': None,
            'corplist_reg': re.compile(r'<dd class="w271">[^<]*<a href=\'(?P<corp_code>[^\']+)\' target="_blank" class="fl" title="(?P<name>[^"]+)', re.M),
            'corp_regs': [
                re.compile(r'^\s+<th>联系人</th>[^>]+>\s+(?P<contact_person>[^\s]+)', re.M),
                re.compile(r'^\s+<th>公司地址</th>[^>]+>[^>]+>(?P<addr>[^<]+)', re.M),
            ],
